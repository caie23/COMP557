...
//	void draw(const std::shared_ptr<Program> prog2, const std::shared_ptr<MatrixStack> MV, float* frameData) const;
    void draw(const std::shared_ptr<Program> prog, const std::shared_ptr<MatrixStack> P, const std::shared_ptr<MatrixStack> MV, glm::fvec3 acc_offset, shared_ptr<Shape> shape, float* frameData ) const;
